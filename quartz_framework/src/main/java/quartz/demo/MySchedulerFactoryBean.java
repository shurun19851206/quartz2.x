 ...
 */
public class MySchedulerFactoryBean extends SchedulerAccessor implements FactoryBean<Scheduler>,
	...
	private void ff (Scheduler scheduler) throws SchedulerException {
		JobDetail jobDetail = JobBuilder.newJob(MyTask1.class).withIdentity("testJob", "testJob").build();
		jobDetail.getJobDataMap().put("testJob", jobDetail);
		CronScheduleBuilder scheduleBuilder = CronScheduleBuilder.cronSchedule("0/5 * * * * ?");
		CronTrigger trigger = TriggerBuilder.newTrigger().withIdentity("testTrigger", "testTrigger").withSchedule(scheduleBuilder).build();
