 ...
 */
public class MySchedulerFactoryBeanDemo extends SchedulerAccessor implements FactoryBean<Scheduler>,
	...
	private void ff (Scheduler scheduler) throws SchedulerException {
		JobDetail jobDetail1 = JobBuilder.newJob(MyDemo1Task1.class).withIdentity("testJob1", "testJob1").storeDurably(true).build();
		jobDetail1.getJobDataMap().put("testJob1", jobDetail1);
		
		JobDetail jobDetail2 = JobBuilder.newJob(MyDemo1Task2.class).withIdentity("testJob2", "testJob2").storeDurably(true).build();
		jobDetail2.getJobDataMap().put("testJob2", jobDetail2);
		
		CronScheduleBuilder scheduleBuilder1 = CronScheduleBuilder.cronSchedule("0/5 * * * * ?");
		CronTrigger trigger1 = TriggerBuilder.newTrigger().withIdentity("testTrigger1", "testTrigger1").withSchedule(scheduleBuilder1).build();
		
		CronScheduleBuilder scheduleBuilder2 = CronScheduleBuilder.cronSchedule("0/5 * * * * ?");
		CronTrigger trigger2 = TriggerBuilder.newTrigger().withIdentity("testTrigger2", "testTrigger2").withSchedule(scheduleBuilder2).build();
		
		scheduler.scheduleJob(jobDetail1, trigger1);
		scheduler.scheduleJob(jobDetail2, trigger2);
		
//		CronScheduleBuilder scheduleBuilder3 = CronScheduleBuilder.cronSchedule("0/15 * * * * ?");
//		CronTrigger trigger3 = TriggerBuilder.newTrigger().withIdentity("testTrigger3", "testTrigger3").withSchedule(scheduleBuilder3).build();
		
		// 暂停Job
//		scheduler.pauseJob(jobDetail1.getKey());
		
		// 恢复Job
//		scheduler.resumeJob(jobDetail1.getKey());
		
		// 
		
		
//		scheduler.rescheduleJob(trigger2.getKey(), trigger3);
	}
